...
async function ensureEncryptedPayslipColumns(connection) {
  const required = [
    ...
    'net_pay_encrypted',
    'source_summary_encrypted',
    'payslip_storage_encrypted_at',
  ];
...
async function encryptedStorageRows(connection) {
  const [rows] = await connection.execute(
    `SELECT id, total_earning, total_deduction, net_pay, source_summary,
            total_earning_encrypted, total_deduction_encrypted, net_pay_encrypted,
            source_summary_encrypted
       FROM payslips
...
async function encryptedBackfillRows(connection) {
  const [rows] = await connection.execute(
    `SELECT id, total_earning, total_deduction, net_pay, source_summary,
            total_earning_encrypted, total_deduction_encrypted, net_pay_encrypted,
            source_summary_encrypted
       FROM payslips
      WHERE total_earning_encrypted IS NOT NULL
         OR total_deduction_encrypted IS NOT NULL
         OR net_pay_encrypted IS NOT NULL
         OR source_summary_encrypted IS NOT NULL`
  );
  return rows;
}
...
exports.up = async function up() {
  ...
  try {
    ...
    const rows = await encryptedStorageRows(connection);
      ...
      await connection.execute(
        `UPDATE payslips
            SET total_earning_encrypted = COALESCE(total_earning_encrypted, ?),
                ...
                source_summary_encrypted = COALESCE(source_summary_encrypted, ?),
                total_earning = NULL,
        ...
        [
          ...
          encryptColumnValue(row.source_summary),
          row.id,
        ]
      );
    }
...
exports.down = async function down() {
  ...
  try {
    await ensureEncryptedPayslipColumns(connection);
    const rows = await encryptedBackfillRows(connection);
    for (const row of rows) {
      await connection.execute(
        `UPDATE payslips
            SET total_earning = COALESCE(total_earning, ?),
                ...
                total_earning_encrypted = NULL,
                total_deduction_encrypted = NULL,
                net_pay_encrypted = NULL,
                source_summary_encrypted = NULL,
                payslip_storage_encrypted_at = NULL
          WHERE id = ?`,
        [
          ...
          decryptColumnValue(row.source_summary_encrypted),
          row.id,
        ]
      );
    }
